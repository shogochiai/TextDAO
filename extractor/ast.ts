...
export class StructMember {
    ...
    static fromASTNode(node: any, index:number, parent: StructDefinition | null): StructMember {
        ...
        let isNaiveStruct:boolean = false;
        ...
        if (!member.type) throw new Error("No type");

        if (isArray || isMapping) {
            member.isMapping = true;
            member.keyType = keyTypeStr;
            member.valueType = new StructMember(node.name, valueTypeStr, index, parent);
        }
...
export class StructDefinition {
    ...
    static fromASTNode(node: any, baseSlots: BaseSlots, parentStruct: StructDefinition | null): StructDefinition {
        const members = node.members.map((memberNode: any, index: number) => {
            return StructMember.fromASTNode(memberNode, index, <StructDefinition>(<unknown>this));
        });
        ...
        if (tempKey in structMap) {
            ...
            if (member.isMapping) {
                ...
                try {
                    ...
                    member.parent = deepCopy(cursorDefinition);
                    childDefinition.parent = deepCopy(cursorDefinition);
                    childDefinition.slotId = member.calculateSlotId();
                }
...
function regexpStruct (str: string) : string[] {
    ...
    return str.match(/^struct\s+\w+\.(\w+)/)?.slice(1, 3) || [];
}